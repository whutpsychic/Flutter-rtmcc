...
class FormMinutePicker extends StatefulWidget {
  final Key? key;
  // 标签
  final String label;
  // 是否是必选项
  ...
  @override
  State<StatefulWidget> createState() => FormMinutePickerState();
}

class FormMinutePickerState extends State<FormMinutePicker> {
  DateTime? _currTime;
  String? _errText;
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Text(
                  _currTime != null
                      ? "${_executeNum(_currTime!.hour)}:${_executeNum(_currTime!.minute)}"
                      : "",
                  style: _enabled
                      ? FormStyle.textStyle
  ...
  _showPicker(bool enabled) {
    if (!enabled)
      Selector.of(context).showMinute(
        (v) {
